...
def padding_cbc_attack(start_time):
	...
	Ci = bytearray.fromhex('0000000000000000')
	P  = bytearray()
	
	#On parcout la chaine du CipherText, on la découpe par block de 8 octets
	for block in range(0, len(CIPHER), 8):
		C  = CIPHER[block:block+8]
		...
		for o in range(7, -1, -1):
			...
			print("\nBlock " + str(int(block/8))  + "/"+ str(int(len(CIPHER)/8)) +" - Octet " + str(o))
		...
		Ci = CIPHER[block:block+8]
		print("P: " + str(P.decode('utf-8')))
